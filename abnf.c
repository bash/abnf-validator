...
static const char *token_name[] = {
    "error", "comment", "ruledef", "amendrule", "hexmode", "binmode",
    "decmode", "alternate", "repeat", "begingroup", "endgroup",
    "beginoption", "endoption", "range", "litlist", "number",
    "hexnumber", "quoted", "prose", "rulename", "newline"
};
...
struct rulename *hashfind(const char *ptr, int len)
{
    ...
    pptr = &htable[hash];
    while (*pptr && ((*pptr)->len != len
		     || strncasecmp(ptr, (*pptr)->data, len) != 0)) {
	pptr = &(*pptr)->next;
    }
...
int main(int argc, char **argv)
{
	...
	*dst = '\0';
	...
	if ((nptr = gettoken(ptr, &tokentype, hexmode)) == NULL)
	    break;
	...
	/* reset hexmode */
	if (hexmode && tokentype != HEXNUMBER && tokentype != LITLIST
	    && tokentype != RANGE) {
	    hexmode = 0;
	}
